...
One particularly important case of such equation is when $q(t) = cos (omega t)$, $ y' + k y = k cos(omega t). #tag[(sinusoidal input)] $ To solve, or better understand this equation, is the "complexify" the equation, motivated by the $cos (omega t)$ term. Due to the superposition principle, if $ y_1' + k y_1 = k cos (omega t) $ and $ y_2' + k y_2 = k sin (omega t), $ then with $tilde(y):= y_1 + i y_2$, $ tilde(y) + k tilde(y)' = k e^(i omega t). $
...
Similarly, $Re(tilde(y))$ may also be found using Cartesian methods along with the following formula, known as _auxiliary angle method_.

#lbl(<thm:aam>, theorem[
  $ a cos theta + b sin theta = C cos (theta - phi) $ where $C = sqrt(a^2 + b^2)$ and $phi = arctan(b/a)$.
])

Note that this method could be verified with trig identity from rhs to lhs. A short, more elegant proof is as following.
#proof[
  Define $vu(u)$ to be the unit vector in the $theta$ direction and $vb(v) = mat(a, b)^top$.
  Lhs may be written as $vu(u) dot vb(v)$. However, with _Law of Cosines_, $ vu(u) dot vb(v) = cos(theta - arctan(b/a)) dot 1 dot sqrt(a^2+ b^2) = "rhs". #qedhere $
]
...
#solution[
  By the superposition principle (linearity), let $ cases(y_1' + k y_1 = 1, y_2' + k y_2 = e^(-5 t)) quad, $ then the solution to the given ODE is $ y = 4 y_1 + 7 y_2. $
